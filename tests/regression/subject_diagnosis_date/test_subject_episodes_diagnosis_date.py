...
def test_amend_diagnosis_date_no_change_with_reason_only_alert(page: Page) -> None:
    ...
    expected = (
    "The date of diagnosis must not be earlier than any previously recorded diagnosis date "
    f"({date.today().strftime('%d %b %Y')}) and not in the future. "
    "Please raise a support call to enter an earlier diagnosis date."
    )
    assert alert_message == expected, f"Unexpected alert message. Got: {alert_message}"

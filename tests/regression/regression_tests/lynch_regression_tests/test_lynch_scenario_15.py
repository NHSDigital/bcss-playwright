...
def test_lynch_scenario_15(page: Page) -> None:
    """
    Scenario: 15 - LNPCP result from diagnostic tests

    G1-G2-G3-A183-A25-J10-A99-A59-A259-A315-A361-A323-A318-A157-Lynch in age [SSCL52c]
    ...
    > Check recall [SSCL52c]

    The scenario also tests:

    > The "Resect & Discard is not appropriate for a Lynch patient" warning message is displayed at the top of the dataset.
    > The "Number of Polyps Resected & Discarded" field is not displayed.
    > When an accredited clinician is selected, and a polyp is added that in a non-Lynch episode would allow Resect & Discard:

    > "No - Resect & Discard" is not available as a Retrieved option.
    > The Optical Diagnosis field is not displayed.
    > The Optical Diagnosis Confidence field is not displayed.
    """
    ...
    logging.info(
        'Check the "not accredited for R&D" message is not displayed in edit mode in a Lynch episode, even for an accredited clinician'
    )
    ...
    # Then no Resect & Discard message is displayed
    InvestigationDatasetsPage(page).assert_resect_and_discard_message(None)
    ...
    # And there "is not" a visible field called "Number of Polyps Resected & Discarded" in the "Investigation Dataset" section of the investigation dataset
    assert InvestigationDatasetsPage(page).are_fields_on_page(
        "Investigation Dataset", None, ["Number of Polyps Resected & Discarded"], False
    ), "'Number of Polyps Resected & Discarded' field is visible, but it should not be in a Lynch episode"

    logging.info(
        "Add a polyp that in an FOBT/Surveillance episode would meet the criteria for Resect & Discard to be a Retrieved option, \n"
        "and the Optical Diagnosis fields to be displayed, and check that they aren't there in this Lynch episode"
    )
    ...
    # Then I confirm the "Retrieved" dropdown on polyp 1, intervention 1, has the following options:
    DatasetFieldUtil(page).assert_select_to_right_has_values(
        "Retrieved", ["Yes", "No"], "divPolypTherapy1_1Section"
    )

    # And there "is not" a visible field called "Optical Diagnosis" on Polyp 1
    assert InvestigationDatasetsPage(page).are_fields_on_page(
        "polyp information",
        "Polyp 1 Intervention 1 Information",
        ["Optical Diagnosis"],
        False,
    ), "'Optical Diagnosis' field is visible, but it should not be in a Lynch episode"

    # And there "is not" a visible field called "Optical Diagnosis Confidence" on Polyp 1
    assert InvestigationDatasetsPage(page).are_fields_on_page(
        "polyp information",
        "Polyp 1 Intervention 1 Information",
        ["Optical Diagnosis Confidence"],
        False,
    ), "'Optical Diagnosis Confidence' field is visible, but it should not be in a Lynch episode"
    ...
    # When I select Outcome of Investigation Complete

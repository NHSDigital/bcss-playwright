...
from pages.screening_subject_search.reopen_lynch_surveillance_episode_page import (
    ReopenLynchSurveillanceEpisodePage,
)
...
def test_lynch_scenario_16(page: Page) -> None:
    """
    Scenario: 16 - LNPCP result from symptomatic procedure, then refer another diagnostic test

    G1-G2-G3-A183-A25-J10-A99-A59-A259-A315-A361-A323-A317-A353-A372-A373-A377-A157-Lynch in age [SSCL52c] A372-A373-A389-A360-A410-A415-A416-A316-A319-A395

    In this scenario, the subject is referred for a symptomatic procedure following a diagnostic test which gives no result.  The result of the symptomatic procedure is LNPCP.
    ...
    > LNPCP Result from Symptomatic Procedure > A373 (2.7)
    > Refer Lynch > A377 (2.7)
    > Process A377 letter batch > A157 (2.1)
    ...
    logging.info("Invite them for diagnostic test and attend the test")
    ...
    logging.info(
        "Progress the episode via post-investigation appointment and referral to symptomatic to closure on A157 LNPCP"
    )
    ...
    # Then I "can" advance the subject's episode for "LNPCP Result from Symptomatic Procedure"
    AdvanceLynchSurveillanceEpisodePage(page).can_advance_episode(
        "LNPCP Result from Symptomatic Procedure", True
    )

    # When I select the advance episode option for "LNPCP Result from Symptomatic Procedure"
    AdvanceLynchSurveillanceEpisodePage(
        page
    ...
    logging.info("Check the subject's data after episode closure")
    ...
    SubjectScreeningSummaryPage(page).click_reopen_lynch_surveillance_episode_button()
    ReopenLynchSurveillanceEpisodePage(
        page
    ...
    subject_assertion(
        nhs_no,
        {
            "which diagnostic test": "Latest not-void test in latest episode",
            "calculated fobt due date": "Null",
            "calculated lynch due date": "As at episode start",
            "calculated surveillance due date": "Null",
            "ceased confirmation date": "Null",
            "ceased confirmation details": "Null",
            "ceased confirmation user id": "Null",
    ...
    # TheAndn my subject has been updated as follows:
    subject_assertion(
        nhs_no,
        {
    ...
    # And there is a "A319" letter batch for my subject with the exact title "Result Letters - Refer another test after symptomatic referral (Lynch)"
    # When I process the open "A319" letter batch for my subject
    batch_processing(
        page,
        "A319",
        "Result Letters - Refer another test after symptomatic referral (Lynch)",
    )

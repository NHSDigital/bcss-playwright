...
def get_subjects_by_note_count(type_id: int, note_count: int = 0) -> pd.DataFrame:
    """
    ...
    params = {"type_id": type_id, "note_count": note_count}
    subjects_df = OracleDB().execute_query(query, params)

    return subjects_df


def get_supporting_notes(screening_subject_id: int, type_id: int) -> pd.DataFrame:
    """
